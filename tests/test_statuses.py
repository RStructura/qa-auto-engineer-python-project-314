...
def test_view_statuses_list(auth_driver):
    ...
    slug = auth_driver.find_element(
        By.CSS_SELECTOR, "tbody tr:first-child td.column-slug"
    ).text.strip()

    assert name, "Name в первой строке пустой"
    assert slug, "Slug в первой строке пустой"
...
def test_delete_status_via_checkbox(auth_driver):
    ...
    assert page.get_statuses_count() > 0, "Список пуст"
